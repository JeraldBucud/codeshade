...
Phase 1 reads local project paths and small known metadata files such as `package.json`. It discovers root metadata such as lock files and build wrappers separately from the bounded source scan, but it does not execute those files. Optional Git awareness reads local branch and NUL-delimited working-tree status using bounded `git` subprocess calls with `shell: false`. CodeShade does not run project scripts, tests, builds, Git hooks or package managers automatically.
